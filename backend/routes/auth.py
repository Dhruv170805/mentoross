"""MentorOS – Auth Routes (Beanie)"""
from fastapi import APIRouter, Depends, HTTPException, status
from datetime import datetime, timezone
from pydantic import BaseModel, EmailStr
from core.security import hash_password, verify_password, create_access_token, create_refresh_token, get_current_user_id, decode_token
...
async def register(req: RegisterRequest):
    ...
    return TokenResponse(
        ...
        user={"id": uid, "email": user.email, "name": user.name, "plan": user.plan, "streak_days": user.streak_days}
    )
...
async def update_me(data: dict, user_id: str = Depends(get_current_user_id)):
    ...
    allowed = {"name", "learning_goal", "avatar_url"}
