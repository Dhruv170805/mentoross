"""MentorOS – Resources, Daily Plans, Analytics, AI Agent Routes (Beanie)"""
...
async def list_resources(
    ...
    q = Resource.find(Resource.user_id == user_id)
    if tag:   q = q.find(Resource.tag == tag)
    if type:  q = q.find(Resource.type == type)
    if saved is not None: q = q.find(Resource.saved == saved)
    resources = await q.sort(-Resource.created_at).to_list()
...
async def update_resource(res_id: str, data: ResourceUpdate, user_id: str = Depends(get_current_user_id)):
    ...
    if not res or res.user_id != user_id: raise HTTPException(404)
    for k, v in data.model_dump(exclude_unset=True).items(): setattr(res, k, v)
    await res.save()
    return map_resource(res)
...
async def get_today_plan(user_id: str = Depends(get_current_user_id)):
    ...
    plan = await DailyPlan.find_one(DailyPlan.user_id == user_id, DailyPlan.date == today)
    if not plan:
        return None
...
async def save_plan(plan_in: DailyPlanCreate, user_id: str = Depends(get_current_user_id)):
    today = datetime.now(timezone.utc).date().isoformat()
    existing = await DailyPlan.find_one(DailyPlan.user_id == user_id, DailyPlan.date == today)
    if existing:
        for k, v in plan_in.model_dump().items(): setattr(existing, k, v)
        await existing.save()
        plan = existing
    else:
...
async def ai_generate_plan(user_id: str = Depends(get_current_user_id)):
    notes = await Note.find(Note.user_id == user_id).sort(-Note.created_at).limit(5).to_list()
    tasks = await Task.find(Task.user_id == user_id, Task.status == TaskStatus.COMPLETED).sort(-Task.created_at).limit(10).to_list()
...
async def evaluate(user_id: str = Depends(get_current_user_id)):
    tasks = await Task.find(Task.user_id == user_id, Task.status == TaskStatus.COMPLETED).limit(20).to_list()
    notes = await Note.find(Note.user_id == user_id).sort(-Note.created_at).limit(10).to_list()
...
async def review_history(limit: int = 10, user_id: str = Depends(get_current_user_id)):
    reviews = await AIReview.find(AIReview.user_id == user_id).sort(-AIReview.created_at).limit(limit).to_list()
...
async def get_active_reminders(user_id: str = Depends(get_current_user_id)):
    """Returns currently active reminders for the user."""
    # Placeholder: In a full implementation, we would query a Reminders collection.
    return []
...
async def analytics_overview(user_id: str = Depends(get_current_user_id)):
    ...
    for i in range(6, -1, -1):
        ...
        weekly.append({"date": day.isoformat(), "label": day.strftime("%a"), "count": day_done})
    ...
    for i in range(90, -1, -1):
        ...
        activity = min(day_done + day_notes, 4)
        heatmap.append({"date": day.isoformat(), "level": activity})
    ...
    # Subject Mastery Calculation via Constants
    subjects_results = []
    ...
    return {
        "mastery": mastery,
        "tasks_total": total, "tasks_done": comp,
        "notes_total": len(notes),
        "weekly": weekly,
        "heatmap": heatmap,
        "streak_cal": streak_cal,
