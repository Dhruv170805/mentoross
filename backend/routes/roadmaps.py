"""MentorOS – Roadmap Routes (Beanie)"""
from fastapi import APIRouter, Depends, HTTPException
from typing import List
from core.security import get_current_user_id
from models.all_models import (
    Roadmap, RoadmapCreate, RoadmapRead, map_roadmap,
    RoadmapPhase, PhaseCreate, map_phase,
    RoadmapTopic, TopicCreate, TopicUpdate, map_topic, utcnow
)
...
async def list_roadmaps(user_id: str = Depends(get_current_user_id)):
    roadmaps = await Roadmap.find(Roadmap.user_id == user_id, Roadmap.is_active == True).sort(-Roadmap.created_at).to_list()
    result = []
    for rm in roadmaps:
        phases = await RoadmapPhase.find(RoadmapPhase.roadmap_id == str(rm.id)).sort(+RoadmapPhase.order_index).to_list()
        all_topics = []
        phases_data = []
        for ph in phases:
            topics = await RoadmapTopic.find(RoadmapTopic.phase_id == str(ph.id)).sort(+RoadmapTopic.order_index).to_list()
...
async def add_phase(roadmap_id: str, ph_in: PhaseCreate, user_id: str = Depends(get_current_user_id)):
    ...
    existing = await RoadmapPhase.find(RoadmapPhase.roadmap_id == roadmap_id).to_list()
    order = len(existing)
...
async def delete_phase(phase_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    topics = await RoadmapTopic.find(RoadmapTopic.phase_id == phase_id).to_list()
    for t in topics:
        await t.delete()
    await ph.delete()
...
async def add_topic(phase_id: str, topic_in: TopicCreate, user_id: str = Depends(get_current_user_id)):
    ...
    if not ph:
        raise HTTPException(status_code=404)
    existing = await RoadmapTopic.find(RoadmapTopic.phase_id == phase_id).to_list()
