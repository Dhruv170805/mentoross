...
async def list_tasks(
    ...
    tasks = await Task.find(query).sort("-created_at").skip(offset).limit(limit).to_list()
    return [map_task(t) for t in tasks]


@router.post("/", response_model=TaskRead, status_code=201)
async def create_task(task_in: TaskCreate, user_id: str = Depends(get_current_user_id)):
...
async def task_stats(user_id: str = Depends(get_current_user_id)):
    ...
    total = len(all_tasks)
    done  = [t for t in all_tasks if t.status == TaskStatus.COMPLETED]
    ...
    return {"total": total, "done": len(done), "pending": pending,
            "completion_pct": pct, "weekly": weekly}
...
async def update_task(task_id: str, task_in: TaskUpdate, user_id: str = Depends(get_current_user_id)):
    task = await Task.get(PydanticObjectId(task_id))
    if not task or task.user_id != user_id:
        raise HTTPException(status_code=404, detail="Task not found")
